...
public partial class TimeSpanPicker : Control
{
    ...
    [MemberNotNullWhen(true, nameof(_daysNumericTextBox),
        nameof(_hoursNumericTextBox),
        nameof(_minutesNumericTextBox),
        nameof(_secondsNumericTextBox),
        nameof(_editorNumericTextBox),
        nameof(_daysAbbreviationTextBlock),
        nameof(_hoursAbbreviationTextBlock),
        nameof(_minutesAbbreviationTextBlock),
        nameof(_secondsAbbreviationTextBlock),
        nameof(_daysHoursSeparatorTextBlock),
        nameof(_hoursMinutesSeparatorTextBlock),
        nameof(_minutesSecondsSeparatorTextBlock),
        nameof(_editedUnitTextBlock),
        nameof(_numericTbEditorContainerBorder)
        )]
    ...
    private void OnEditorValueChanged(object? _, EventArgs e)
    {
        if (!IsPartsInitialized)
        {
            return;
    ...
    private void OnDaysValueChanged(object? _, EventArgs args)
    {
        var days = Days;
        var value = Value;
        if (value is null)
    ...
    private void OnHoursValueChanged(object? _, EventArgs args)
    {
        var hours = Hours;
        var value = Value;
        if (value is null)
    ...
    private void OnMinutesValueChanged(object? _, EventArgs args)
    {
        var minutes = Minutes;
        var value = Value;
        if (value is null)
    ...
    private void OnSecondsValueChanged(object? _, EventArgs args)
    {
        var seconds = Seconds;
        var value = Value;
        if (value is null)
    ...
    private void OnAbbreviationTextBlockOnMouseDown(object? sender, MouseButtonEventArgs e)
    {
        ...
        _activeTextBoxPart = (TimeSpanPart)textBlock.Tag;
    ...
    {
        if (sender is not NumericTextBox numericTextBox)
        {
            return;
        }
